...
class TrainValDataset(Dataset):
    ...
    def get_imgs_labels(self, img_dir):
        ...
        label_paths = sorted(osp.join(label_dir, osp.splitext(osp.basename(p))[0] + ".txt") for p in img_paths)
        ...
        if self.check_labels:
            ...
            with Pool(NUM_THREADS) as pool:
                pbar = pool.imap(TrainValDataset.check_label_files, zip(img_paths, label_paths))
    ...
    def sort_files_shapes(self):
        ...
        for i in range(batch_num):
            ...
            if maxi < 1:
                ...
                shapes[i] = [1, 1 / mini]
        self.batch_shapes = (
            np.ceil(np.array(shapes) * self.img_size / self.stride + self.pad).astype(np.int) * self.stride
        )
