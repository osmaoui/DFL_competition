    ...
    def print_details(self):
        if self.main_process:
            ...
            self.pbar.set_description(
                ("%10s" + "%10.4g" * 4) % (f"{self.epoch}/{self.max_epoch - 1}", *(self.mean_loss))
            )
    ...
    def update_optimizer(self):
        ...
        if curr_step <= self.warmup_stepnum:
            ...
            for k, param in enumerate(self.optimizer.param_groups):
                ...
                param["lr"] = np.interp(
                    curr_step, [0, self.warmup_stepnum], [warmup_bias_lr, param["initial_lr"] * self.lf(self.epoch)]
                )
                if "momentum" in param:
                    param["momentum"] = np.interp(
                        curr_step, [0, self.warmup_stepnum], [self.cfg.solver.warmup_momentum, self.cfg.solver.momentum]
                    )
