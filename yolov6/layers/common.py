...
class IdentityBasedConv1x1(nn.Conv2d):
    def __init__(self, channels, groups=1):
        super(IdentityBasedConv1x1, self).__init__(
            in_channels=channels, out_channels=channels, kernel_size=1, stride=1, padding=0, groups=groups, bias=False
        )
...
class DBBBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n=1):
        ...
        self.block = (
            nn.Sequential(*(DiverseBranchBlock(out_channels, out_channels) for _ in range(n - 1))) if n > 1 else None
        )
